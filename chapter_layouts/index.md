(chap_layouts)=
# Tile Primitives: Scope, Layout, and Dispatch
...
TIRx names the execution level with a scope prefix on the tile op. In the snippets below, focus on the `Tx.<scope>.` prefix that names the scope; the tile names (`Asmem`, `tmem`, `Dreg_wg`) are just stand-ins, defined for real in the GEMM chapters:

```python
Tx.cta.copy(Asmem[:, :], A[m:m + BLK_M, k:k + BLK_K])
...
A scope prefix does not create new physical threads. The CUDA launch already did that. The scope tells TIRx how to interpret the prefixed primitive.

For example, `Tx.cta.copy` is a cooperative CTA-level copy. A bare `Tx.copy` is a per-thread copy. A TMEM-to-register readback must use `Tx.wg.copy_async` because the corresponding `tcgen05.ld` instruction is warpgroup-cooperative.

The scopes are not a fixed template that every kernel must use exactly. A kernel uses the scopes that match the operations it performs. `T.device_entry()` marks the start of device code; the `Tx.cluster.` prefix appears only when CTA clusters are used.
...
Read it from the outside in. `T.cta_id(...)` gives the CTA's position in the launch grid, so the kernel can choose an output tile such as `D[bx * BLK_M, by * BLK_N]`. `T.warpgroup_id([1])` declares the warpgroup coordinate for a single-warpgroup kernel; later kernels use more warpgroups when producer, consumer, and writeback roles are split. `T.warp_id_in_wg([4])` and `T.lane_id([32])` identify a thread inside one warpgroup, which is enough to assign output rows during writeback.

The number inside brackets is the extent of that coordinate space. For example, `[4]` in `T.warp_id_in_wg([4])` says there are four warps in the warpgroup, and `[32]` in `T.lane_id([32])` says each warp has 32 lanes. TIRx uses these extents when checking layouts and tile primitives. A warpgroup register layout that uses `tid_in_wg` has extent 128, so it matches a 128-row TMEM readback; a lane-only layout has extent 32, so it would describe a different mapping.

Other coordinates appear when the kernel needs them. `T.thread_id([Nt])` names a thread inside a CTA. `T.thread_id_in_wg([128])` names a thread inside a warpgroup. Clustered kernels add `T.cluster_id(...)` and `T.cta_id_in_cluster(...)` to identify the cluster and the CTA's position inside it.
...
The path is implied by the operands and scope: source in TMEM, destination in a warpgroup-distributed register view, and warpgroup scope (the `Tx.wg.` prefix). That combination lowers to the Blackwell TMEM load path, `tcgen05.ld`.
...
| API | How to read it |
|-----|----------------|
| `@T.prim_func` | Define one TIRx kernel function. |
| `T.Buffer(shape, dtype)` | Declare a typed device buffer argument. |
| `T.SMEMPool()` | Create a shared-memory allocation pool for this kernel. |
...
| `T.ptx.tcgen05.alloc(...)` | Ask Blackwell hardware for a TMEM allocation. |
| `T.decl_buffer(..., scope="tmem", allocated_addr=...)` | Create an indexable TIRx view of an existing TMEM allocation. |
| `T.alloc_local(shape, dtype)` | Allocate per-thread register storage. |
| `T.wg_reg_tile(elem_per_thread, dtype=...)` | Allocate a warpgroup-distributed `(128, elem_per_thread)` register tile for TMEM readback. |
| `T.cuda.cta_sync()` | Synchronize all threads in the CTA and order shared-memory writes. |
| `T.ptx.mbarrier.*` | Use raw mbarrier operations for async completion. |
| `T.ptx.tcgen05.*` | Use raw `tcgen05` helpers such as TMEM allocation, MMA commit, and wait. |
| `T.ptx.fence.*` | Add ordering across async/proxy boundaries. |
| `T.meta_var(expr)` | Keep an index expression inline in generated TIR. |

Two patterns are worth spelling out before the GEMM code.

First, shared memory is packed through `T.SMEMPool()`. The first allocations are usually small control objects:
...
Dreg_wg = T.wg_reg_tile(BLK_N)

Dreg_f16 = T.alloc_local((BLK_N,), d_type)
```

Either way, `Dreg_wg` is the warpgroup-distributed register tile that receives the fp32 accumulator read from TMEM via `tcgen05.ld`; its `(1@tid_in_wg, 1)` layout is exactly the readback view described above. What the hardware path checks is that layout, not whether you wrote `T.wg_reg_tile` or `T.alloc_local(...).view(...)`. `Dreg_f16` is per-thread local storage for the casted fp16 values that will be stored to output memory.

`T.meta_var(...)` appears often in address calculations:
...
1. For a TMEM-to-register readback, name its scope, its source and destination layouts, and its dispatch path. Why must it use warpgroup scope (`Tx.wg.`) and not per-thread (a bare `Tx.` op)?
