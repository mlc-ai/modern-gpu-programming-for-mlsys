...
engine a descriptor — the tile shape, strides, and destination — and the engine streams the bytes on
its own.
...
without bank conflicts. The swizzle pattern travels in the TMA descriptor, and it must match the
