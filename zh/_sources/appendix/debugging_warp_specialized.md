...
- **Barrier init 位于 `wg_id` guard 内。** `.init()` 会 lowering 为 `if threadIdx.x < 1:`，也就是 CTA thread 0。CTA thread 0 位于 WG0，因此放在 `if wg_id == 1:` 内会导致所有 threads 都无法执行 init。Init 必须位于顶层；可以在 `inspect_source()` 中搜索 `mbarrier_init` 验证。
...
- **部分 consumer warpgroup threads 跳过了 `tile_scheduler.next_tile()`。** Scheduler 保存 per-thread 状态；跳过调用的 threads 可能永远留在 loop 中。
...
- **Persistent kernel 在 1024x1024 等较小 shape 上偶发失败。** 更大的 shape 和更长的 K-loop 可能掩盖竞争。重新检查 tiles 之间的 phase reset 和 TMA store commit/wait。
