...
先看一个表面上的矛盾。普通 MHA 会为每个 token 保存 $n_h$ 份互不相同的
head 切片；K 或 V 的总宽度各为 $n_h d_h$。本章研究的 Blackwell kernel
接受 128 个 query heads，但它的 KV tensor 却只有一个 head。一条 cached KV
entry 必须同时服务这 128 个 heads，保留各 head 不同的 attention 行为，并
产生 128 份各含 512 个 coordinates 的 output。

这个问题分为三层。MLA 解释 cache 为何可以共享；weight absorption 解释各 head
的 projection 去了哪里；FlashMLA kernel 则解释如何把共享 cache rows 的稀疏
列表变成 2-CTA 协作的 dense tensor-core computation。

```{figure} ../../img/flashmla_cache_story_zh.svg
:width: 100%
:alt: 一条 latent KV cache entry 通过吸收投影服务 128 个 query heads

缓存为每个 token 保存一份 latent content vector 和一份共享 RoPE key。各
head 的行为保留在 query 与 output 路径中，而不是展开成 128 份 K/V cache
entries。
```
...
- 理解 MLA 如何共享 latent KV cache、MHA/MQA 两种执行模式，以及 weight absorption。
- 从 sparse contract 追踪到 Blackwell 上由 2 个 CTA 协作完成的数据流程序。
- 运行可执行 reference，并在 B200 上编译、验证固定版本的 TIRx 实现。
:::

*FlashMLA* 这个名字包含两个容易混淆的概念。MLA 是一种 attention 架构，
它将 key-value 状态压缩成 latent vector；FlashMLA 则是 DeepSeek 提供的
高性能 attention kernel library，其中既包含 dense 和 sparse operator，也
覆盖 prefill 与 decode。

本章会先推导 MLA 架构，再让 sparse contract 真正运行起来，最后沿用 FA4
从算法深入 schedule 的分析方式，研究一条具体的 library 路径：Blackwell 上
regular 128-query-head sparse-prefill forward kernel。

本章实现固定使用
[`mlc-ai/tirx-kernels@5be39749`](https://github.com/mlc-ai/tirx-kernels/tree/5be39749e7dfd2c4bdae9b4d396f8ec35af07126)。
下文所有 TIRx API 写法和代码摘录都来自这个 revision。这一点很重要：
compiler API 仍在演进，而教材示例必须能够复现。

## 为什么 MHA 要为每个 head 分别缓存 K/V 状态？

设 $h_t\in\mathbb{R}^{d_{model}}$ 是 token $t$ 的 hidden state。普通
multi-head attention（MHA）会为每个 head $i$ 分别做 projection：
...
MLA 还可以引入一个独立的低秩 latent，对 query projection 做分解：

$$
c_t^Q=W^{DQ}h_t,\qquad q_t^C=W^{UQ}c_t^Q.
$$

Query 侧的这项分解会降低训练时的 activation memory，但不会进一步缩小 KV
cache。

它也发生在 core-attention API 之前：传给 FlashMLA 的 `q` tensor 已经是
projection 后的 query。因此，下文把 $q^C$ 视为输入，重点讨论决定 cache 与
kernel operands 的 KV 侧 compression。

RoPE 让这幅简单图景变得复杂。如果在 $W^{UK}$ 与 dot product 之间加入
...
MLA 的 core attention 可以用两种代数上相关的模式执行。不能因为其中一种
叫 MQA mode，就把所有 MLA 模型都理解成普通 MQA 模型。
...
mode，在 decode 时使用 MQA mode。相比之下，DeepSeek Sparse Attention
（DSA）即使在 sparse prefill 中也采用 MLA 的 MQA mode，因为每个被选中的
latent KV entry 会由所有 query heads 共享。因此，这条 sparse-prefill 路径
中常见的 $d_{qk}=576$、$d_v=512$ contract 表示吸收权重后的 MQA
representation：576 这一变体由 512 个 latent content coordinates 和 64 个
RoPE coordinates 组成。它不是所有 MLA 或 FlashMLA operator 的通用 shape；
实现还支持 $d_{qk}=512$ 变体。
...
## Sparse kernel 会自己选择 token 吗？

Dense attention 会访问每个合法的 KV token。DSA 先用轻量的 *lightning
indexer* 为候选 tokens 打分，并为每个 query 选出一个 top-$k$ 集合；sparse
...
:width: 100%
:alt: Lightning indexer 先选择 token，sparse-prefill kernel 再执行 attention

Token selection 与 sparse core attention 是两个相互独立的算子。Indexer
产生 row addresses；sparse-prefill kernel gather 这些 rows，再完成
QK--softmax--PV 计算。
```
...
如果提供 `topk_length`，每个 query 的值都必须满足
`0 <= topk_length[q] <= topk`。这是 caller 应保证的前置条件，而不是
kernel 会自动 clip 的输入；大于 `topk` 会让固定版本的 kernel 走到
`indices` storage 之外。

这个 prefill interface 也没有 batch dimension。在下文研究的 port 中，
`h_kv=1`，所以 `indices` 的 shape 是 `[s_q, 1, topk]`：每个 query token
提供一份 selected-token list，它的 128 个 query heads 共用这份 list。Serving
system 必须在调用前 flatten batch，或用其他方式完成 batch mapping。

### FlashMLA 是一个 kernel，还是一族算子？

*Prefill* 并行处理 prompt 中的 query tokens；*decode* 则在复用 KV cache 的
同时，每一步加入一个新 query（或一小组 speculative queries）。
[FlashMLA 官方仓库](https://github.com/deepseek-ai/FlashMLA)将 kernel 组织成
四类：
...
所以，FlashMLA 既不等同于 sparse attention，也不等同于本章这个 kernel。

本章选择 sparse prefill operator，是因为它把一个系统问题展示得很清楚：
irregular TMA gather 如何为规则的 tensor-core tiles 供数？Sparse decode 有
不同的 paged-cache、scheduling 和 reduction contract，不在本章范围内。

Upstream sparse-prefill public call 在概念上是：
...
固定版本的 TIRx repository 在统一的 `flash_mla_sparse_fwd` registry entry 中
沿用了这个 public name，再根据 shape dispatch 到三个 SM100 phase-1 modules
之一。

:::{admonition} Port 范围：`sm_scale` 已固定
:class: warning

上面的 upstream call 会在 runtime 接收 `sm_scale`。固定版本的 TIRx entry
保留了 registry name 和 shape dispatch，却**没有**保留完整的 call
signature：三个 prefill modules 都将 scale specialize 为
`1 / sqrt(d_qk)`，launch ABI 中没有 scale argument。在这个 revision 中，
通过 `**kwargs` wrappers 传入 `sm_scale=...` 会被静默忽略。因此，下文
的 B200 示例验证的是固定 scale 的教学 port，而不是 upstream
`flash_mla_sparse_fwd` 的 drop-in replacement。若模型语义中的 QK scale
不同，就必须将正确数值暴露为参数，或在编译时将其 specialize。
...
阅读 kernel 之前，先写一个明确处理各种边界情况的 reference。在吸收权重后的
MQA contract 中，`kv[:, 0, :]` 同时提供 K 和 V：全部 `d_qk` coordinates
参与 QK，前 `d_v` 个 coordinates 则作为 latent value。PyTorch gather 之前
必须先 clamp invalid indices，再将其 mask 掉；如果直接用 `-1` 做 index，就会
错误地选择最后一行。

越界地址对应的 V row 还要在 PV 前清零，因为按照 IEEE arithmetic，softmax
weight 为 0 并不能消除 NaN。对于只因 `topk_length` 而被 mask、地址本身仍合法的
row，这里有意不做净化，以匹配后文说明的固定版本 kernel 行为。

下面的完整 CPU block 回答这些规则、attention sink 和全 invalid 约定能否共存
于一个可执行 oracle 中：
...
Attention sink 等价于额外加入一个 logit，并令与它关联的 value vector 为 0。
它只改变 output denominator：
...
其中 $a_i$ 是 per-head sink logit。它不影响 `max_logits` 或返回的 `lse`；
编写独立 oracle 时很容易漏掉这个细节。
...
虽然内部的 `phase1` 名称继承自 upstream FlashMLA，但这条 prefill 路径会在
一个 kernel 中生成完整的 `(out, max_logits, lse)`；它不是等待 combine
kernel 的 partial split-KV output。
...
probability；而在固定版本的源码里，`tmem_p` 和 register 变量 `p` 保存的是
...
下面较短的 TIRx 代码块都是固定版本 kernel 的上下文摘录，不是 standalone
program。最后一节会编译并做数值验证；可以独立执行的 blocks 会显式说明。

### 哪些 shape 会进入 regular head-128 路径？

Regular head-128 module 固定使用以下 tile constants：
...
这个 module 接受 512 或 576 的 `d_qk`，要求 `h_kv=1`、`d_v=512`，并要求
regular path 的 `topk` 是 128 的正整数倍。对于 128 heads，统一 front door 还会
多做一次选择：`d_qk=512` 且 `topk<=1280` 时进入 small-top-k 实现；其他
支持的 head-128 shapes 进入本章的 regular 实现。Head-64 shapes 使用另一个
kernel。

`topk > 0` 是调用者必须满足的前置条件。统一 front door 会拒绝
`topk<=0`，但固定版本各 specialization 的 `_cfg().validate()` 只检查整除性，
没有检查正数条件。因此，直接 import 某个 specialization 时仍须拒绝或避开
非正的 `topk`；local validator 接受它并不意味着这种 launch 有效。

无需 launch GPU kernel 就能检查 dispatch：
...
Launch grid 包含 `2 * s_q` 个 CTA，并将相邻 CTA 两两组成 cluster：
...
因此，一个 cluster 负责一个 query row。每个 CTA 含 4 个 warpgroups。先看
ownership 图。这里最不寻常的是，QK 与 PV 之间的切分轴会发生变化。
...
QK 需要形成 128 heads 与 128 selected tokens 的 cross-product，PV 再沿 token
维度做 contraction，得到 512 个 value coordinates。这两次 GEMM 之间的 partition
会发生旋转；collective `cta_group=2` MMA、配对的 SMEM/TMEM layouts 和
cross-CTA barriers 共同组成一个 logical tile。

这个划分可以直接从固定版本源码看出：Q 按 `cta_idx` chunk，K producer 选择
每个 top-k block 的 `cta_idx` 半块，V producer 则从 `cta_idx * 256` 开始。

## 各个 tile 放在哪里？

Blackwell tensor memory（TMEM）让 `tcgen05` 可以把大型 accumulator 放在
tensor cores 附近，但它不会取代 shared memory（SMEM）。TMA gather 的数据先
到达 SMEM，softmax warpgroup 也必须在 SMEM 中 materialize BF16 未归一化
权重，供 PV GEMM 使用。

```{figure} ../../img/flashmla_dataflow_zh.svg
:width: 100%
:alt: Global memory、shared memory、tensor memory 与 registers 之间的数据移动
...
这些都是 logical views；CTA-group TMEM layout 和 rearrangement 决定 MMA 与
load/store instructions 的实际 lane mapping。源码分配一个 512-column
CTA-group TMEM pool，再从中切出 O、raw-logit 和 Q views。
...
四个 warpgroups 各做不同工作，而不是 lockstep 前进：
...
Register budget 也与 role 对应。WG0 将上限提高到 144 registers，WG3 提高到
168；producer groups 则降到 96。固定版本 API 用
`T.ptx.setmaxnreg(True, ...)` 表示提高，用
`T.ptx.setmaxnreg(False, ...)` 表示降低。
...
它的 completion 又交给哪个 barrier。Index names 和 slices 与固定 revision
完全相同：
...
与 upstream API 相同，caller 仍应避免让 `topk_length` 之后的 in-range KV
rows 含有 NaN；优化后的数据路径不保证屏蔽这类异常输入。

固定版本源码清楚分开了这些 roles：[K producer 位于 lines
...
下面的源码摘录回答 kernel 如何只让一部分 Q 留在 SMEM。QK dot product 在
$d_{sq}=d_{qk}-384$ 处分成两部分：
...
对于连续到来的 score tiles，普通 online-softmax recurrence 会保存 row origin
$m$、denominator $\ell$ 和未归一化 output $\widetilde O$。使用 base 2 时，
若下一块 tile 含有 scaled scores $r$，则：
...
概念上，实现会将 natural-unit logits 乘以模型语义中的 QK scale 与
`log2(e)`，再使用 `exp2`。在本章的固定 scale port 中，编译期
...
如果新 tile maximum 比保存的 origin 至多高 6 个 base-2 units，kernel 会继续
保留旧 origin。新的 exponential 此时最大可能达到 $2^6=64$，但 accumulated
O 无需 rescale。一旦差值超过 6，kernel 就执行 rebase，同时 rescale $\ell$
和已经存在的 O。Warp-wide `any_sync` 让参与计算的 rows 使用一致的决策。

`real_mi` 单独维护截至当前看到的精确 maximum，所以这项优化不会改变
`max_logits`：`mi` 是 recurrence 使用的 numerical origin，`real_mi` 则是
最终报告的统计量。结束时，两个 64-token half 对每个 logical row 的贡献会被
合并，kernel 输出：
...
`max_logits=-inf`、`lse=+inf`，与固定版本 reference 一致。

## 各个阶段如何安全重叠？

把这个 schedule 看成一个依赖关系问题最容易理解。QK 必须先完成，softmax
才能消费它的 logits；softmax 必须先产生 weights，PV 才能开始。但是，只要
原位复用的 K 或 V segment 已经安全释放，负责 gather 的 warpgroups 就应该
继续向前推进。

```{figure} ../../img/flashmla_pipeline_stages_zh.svg
:width: 100%
:alt: QK、softmax 与 PV 在相邻 tile 之间的稳态重叠

Softmax($k-1$) 可与 QK($k$) 重叠；唯一的 MMA issuer 串行发出 QK($k$) 与
PV($k-1$)；随后 softmax($k$) 可与异步执行的 PV($k-1$) 重叠。这不是两条
同时发射 QK 与 PV 的 tensor-core streams。
```

Kernel 先由 warp 0 初始化 mbarriers，执行 cluster sync，launch Q prologue，
分配 CTA-group TMEM，再进入 specialized loops。

CTA-group gather 的 TMA completion 会被路由到指定的 leader barrier，使
CTA pair 发出的操作共同满足同一 expected byte count。

把主要 barrier edges 写成 ownership transfer 会更容易理解：
...
这样，复用的 barrier slot 可以区分本轮的新 arrival 与两轮之前的旧 arrival。
...
:width: 100%
:alt: Sparse-prefill pipeline，其中当前 tile 执行 QK，前一个 tile 执行 PV
...
这些 edges 周围会出现两类 fence。`T.ptx.tcgen05.fence.*` 对 TMEM access 与
thread-visible work 做 ordering；`T.ptx.fence.proxy_async("shared::cta")` 则在
SMEM 的 generic access 与 asynchronous access 之间建立 cross-proxy ordering。

这里既包括 generic store 写入 $W$ 或 epilogue tile 后，tcgen05/TMA 执行
asynchronous read，也包括 asynchronous SMEM read 完成后，generic code 覆写
alias storage。

Mbarrier 传达 completion 与 ownership handoff；proxy fence 则对不同 proxy 的
memory effects 做 ordering，两者不能互相替代。

## 如何编译并验证整条路径？

固定版本 module 面向 compute capability 10，其 TMA/tcgen05 形式要求 SM100
class GPU。环境应使用 B200、CUDA 12.9 或更高版本，以及官方 Apache TVM
0.26.0 package。

Kernel 使用的是这个 release 的 TIRx API，而不是持续变化的 development
checkout 写法。
...
随后安装 TVM、固定 kernel repository，再运行示例：
...
运行误认为完整 API coverage。

## 哪五条不变量定义了这条路径？

1. **缓存不变量（cache invariant）。** 一条 `h_kv=1` 的 latent KV row 可以
   服务 128 个 query heads，是因为 key up-projection 已吸收到各 head 的 query
   路径，而 value up-projection 被移到 core attention 之后。RoPE channel 仍
   保持显式，QK scale 仍是模型语义规定的 scale。

2. **稀疏契约不变量（sparse-contract invariant）。** Token selection 发生在
   这个 kernel 之前；`indices` 提供 rows，重复项仍按重复项计算，因果合法性由
   caller 保证，并且每个 `topk_length` 都必须位于 `[0, topk]`。优化路径与
   reference path 必须保持相同的 attention sink 和全 invalid 约定。

3. **所有权不变量（ownership invariant）。** 一个 2-CTA cluster 负责一个
   query row。这对 CTA 沿不同的轴切分 Q/output heads、selected K rows 和 V
   features；CTA 0 的 warp 12 是 CTA-group MMA 的唯一 issuer。

4. **驻留不变量（residency invariant）。** $L$ 表示 FP32 原始 logits，$W$
   表示 BF16 未归一化指数权重。K、V、$L$ 和 $W$ 的大型 workspace 都是原位
   复用的单份 tile，而 `NUM_BUFS=2` 驱动的是 barrier/phase ring。在数据缓冲区
   （data buffers）中，只有小型 packed-validity mask 才有两个 physical slots。

5. **交接不变量（handoff invariant）。** Ready/done barriers 转移每个可复用
   segment 的 ownership，proxy fences 则对 generic 与 asynchronous SMEM
   access 排序。唯一 issuer 保证 QK($k$) 先于 PV($k-1$) 发射，而 threshold
   为 6 的优化保证上报 maximum 的语义不变，并保持 online LSE 的递推关系。

前两条不变量定义 operator semantics，后三条定义这个 specialization 的
schedule contract。其他 dispatch 可以修改 tile sizes、register budgets、
ownership 或 barrier topology，但必须显式给出替代 contract，而不能悄悄改变
operator 所计算的结果。

## 接下来应该测试什么？

Regular head-128 kernel 只是 dispatch space 中的一个点。固定版本源码树还包含
64-head phase-1 kernel，以及 128-head `d_qk=512` small-top-k kernel。它们使用
不同 schedule，这说明 sparse attention 应根据 tile economics 做 dispatch，
而不应被强行塞进一个 universal template。
...
5. **测量 threshold。** 对 random 和 adversarial logits 加入 instrumentation，
   ...
   never-rebase 三个版本的 numerical error 与 TMEM O traffic。

6. **比较 dispatch。** 对 head count 64/128、`d_qk` 512/576，以及 1280 附近的
   top-k values 调用 `select_kernel`。运行前先预测选择的 module，再检查哪些
