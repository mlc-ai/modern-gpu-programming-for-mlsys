...
- `CUtensorMap`（CUDA Driver API 中称为 tensor map descriptor）说明 global tensor 如何组织，包括各维 dimensions 和 strides、bounding-box dimensions 以及 swizzle mode；TMA 指令再给出当前 tile 的 coordinates 和 shared-memory 地址。执行 load 时，TMA 可以在写入 shared memory 的同时应用 swizzle，使 tile 直接采用后续 MMA 所需的布局。
...
第一类是 `CUtensorMap`，CUDA Driver API 将它称为 tensor map descriptor。它描述 global tensor 的数据类型、各维 dimensions 和 strides、bounding-box dimensions，以及写入 shared memory 时使用的 swizzle mode。同一个 descriptor 通常可以供多次 tile copies 复用。
...
Swizzle 只改变 tile 的物理排列，不改变它的逻辑内容。Tensor map descriptor、shared-memory tile layout 和后续 MMA 指令必须描述同一种物理排列（{ref}`chap_data_layout`）。如果 TMA 按 128-byte swizzle 写入，而 MMA 按 linear layout 读取，数据虽然已经到达 shared memory，Tensor Core 仍会把这些字节解释成错误的矩阵元素。

## 用 3D TMA 搬运多个 swizzle atoms

`SWIZZLE_128B` 以 `8 rows × 128 bytes` 为一个重复单元，也就是一个 swizzle atom。地址重排只在 atom 内进行，因此 tensor map bounding box 的最内层连续维度不能超过 128 bytes。对于 `fp16`，这段空间正好容纳 64 个元素。
...
为什么要把 256-byte 的一行拆成两个 128-byte groups？除了满足 tensor map bounding box 的宽度限制，分组还会改变跨行访问落到哪些 shared-memory banks。
...
这时只有 4 个不同的结果，每个 bank sector 被访问两次，形成 2-way conflict。这里的未分组状态只是一个对照，它不符合 `SWIZZLE_128B` 对 tensor map bounding box 的要求。
