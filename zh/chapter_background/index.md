...
Blackwell 上的关键操作并不都由同一组线程发起。TMA copy 由单个 thread 发起，再交给硬件执行；TMEM load 由一个 warpgroup 中的四个 warp 协作完成；`tcgen05` MMA 由一个选定的 thread 提交；2-CTA cooperative MMA 则跨越两个 CTA。
...
TMEM 需要由程序显式管理。Kernel 必须分配和释放 TMEM；MMA 完成后，epilogue 还需要显式地将 accumulator 从 TMEM 读回寄存器。TMEM load 通常由一个 warpgroup 中的四个 warps 协作完成。
