...
实际 kernel 还会受到 C 的读写、L2 reuse、occupancy、register pressure、SMEM bandwidth 和 Tensor Core issue rate 等因素影响。这里的模型虽然简化，但已经说明了 blocking 提高算术强度的原因：同一个 A/B 元素从 global memory 读入后，可以在 tile 内参与更多次乘加。
