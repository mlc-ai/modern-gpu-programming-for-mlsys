...
```text
A/B in SMEM --tcgen05.mma--> C/D in TMEM --tcgen05.ld--> registers
```
...
图底部的 `a_desc` 和 `b_desc` 描述 A、B operand 在 SMEM 中的地址与布局；`idesc` 则描述本次 MMA 的指令形状、数据类型以及 operand major mode。切换 N 或 transpose 设置时，这些指令参数也会随之变化。
...
由一个 thread 发起指令，并不意味着 MMA 只能使用当前 CTA 的资源。指令中的 `cta_group` qualifier 用来指定操作范围：`cta_group::1` 使用当前 CTA 的 TMEM，`cta_group::2` 则同时使用一个 CTA pair 的两块 TMEM。两种情况下都只需要一个 thread 发出指令；CTA pair 的组成和两块 TMEM 的布局会在后面单独介绍。

`tcgen05.mma` 的数据来源和目的位置同样由指令模式决定。A、B 通常通过 matrix descriptor 从 SMEM 中读取；某些变体也允许 A 直接来自 TMEM。C/D accumulator 则保存在 TMEM 中。

因此，这里需要同时考虑两类 layout：

- SMEM layout 决定 Tensor Core 如何解释和读取 A、B；
- TMEM layout 决定 C/D accumulator 如何映射到 TMEM 的 lanes 和 columns。

这两种 layout 位于不同的 memory spaces，但都必须符合当前 `tcgen05.mma` 的 instruction shape、数据类型和 operand major mode。关于这些 layout 的表示方法，可以参考“{ref}`数据布局及其记号 <chap_data_layout>`”一章。
...
TMEM 是二维空间，其两个坐标轴在本书的 layout 记号中分别写作 `TLane` 和 `TCol`。PTX 会根据四项信息选择 accumulator 的 TMEM layout：使用 `cta_group::1` 还是 `cta_group::2`、M 维的大小、A 是稠密矩阵还是结构化稀疏矩阵，以及使用普通的 `tcgen05.mma` 还是 weight-stationary 形式的 `tcgen05.mma.ws`。选定的 layout 再规定逻辑坐标 `(m,n)` 如何映射到 `TLane` 和 `TCol`。

对于某一次具体的 `tcgen05.mma`，`N` 由 instruction descriptor 固定为一个合法值。在本节主要讨论的 `f16`/`bf16` 路径中，`cta_group::1` 支持从 8 到 256、以 8 为步长的 `N`，`cta_group::2` 支持从 16 到 256、以 16 为步长的 `N`；其他数据类型或 operand major mode 可能有更严格的限制。

本书后续的 kernels 通常使用 `f32` accumulator。这里的 `.kind::f16` 表示 A、B 属于 16-bit 浮点类型；A、B 具体使用 `f16` 还是 `bf16`，以及 C/D 使用 `f16` 还是 `f32`，都由 instruction descriptor 分别指定。

下面四张图画的不是某一条具体指令，而是适用于上述多个合法 `N` 的映射规则，因此用符号 `N` 表示这一维。例如，在 `cta_group::1, M=128` 时，`N=16` 得到一个 `128×16` accumulator tile，`N=32` 则得到一个 `128×32` tile；两者都把第 `m` 行映射到 TMEM Lane `m`，区别只是后者沿 TMEM columns 占据的范围更大。

在 `cta_group::2, M=128` 且 A 为稠密矩阵时，规则会将 N 维分成两半。`N=16` 时，两半分别包含 8 列；`N=32` 时，两半分别包含 16 列。N 的具体数值发生了变化，但两半分别映射到 Lane 轴下半部和上半部的规则保持不变。

下面分别看四种常见情况。图中紫色表示 SMEM operands，橙色表示 TMEM accumulator，绿色表示 Tensor Core MMA 的数据路径。
